...
define(['./pecas'], function (GrupoPecas) {
    ...
    */
    ...
    var tempoRestante, estaJogando = false, pontuacao;
     ...
     */
    function PSelecionaCarta() {
        if ($('.mesa-peca-selecionada').length > 1)
            return;
        
        if($(this).hasClass('mesa-peca-selecionada')) {
            $(this).removeClass('mesa-peca-selecionada');
            return;
        }
        
        $(this).addClass('mesa-peca-selecionada');
        if ($('.mesa-peca-selecionada').length == 2)
            setTimeout(PVerificaPar, 700);
    }
	
    function PVerificaPar() {
        var pecas = $(".mesa-peca-selecionada");
		
        var $pecaA = $(pecas[0]).data("pattern"); // $pecaA = peca-
        var $pecaB = $(pecas[1]).data("pattern");
    ...
    return {
        IniciarJogo: function(pDificuldade) {
            PIniciarJogo(pDificuldade);
        },
        FinalizarJogo: function() {
            PPararJogo();
        },
        ObterPontuacao: function() {
            return PObterPontuacao();
        }
    };
});
